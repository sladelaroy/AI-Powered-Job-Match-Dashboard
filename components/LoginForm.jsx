...
export default function LoginForm() {
  ...
  const handleSubmit = async (event) => {
    ...
    if (mode === "login") {
      ...
      const payload = { name, email, password, skills: selectedSkills };
      try {
          ...
          {mode === "register" && (
            <div>
              ...
              <div className="mt-3">
                <p
                  className={`text-sm ${
                    selectedSkills.length < 8 ? "text-gray-500" : "text-red-600"
                  }`}
                >
                  {selectedSkills.length < 8
                    ? `You can select up to 8 skills. You have selected ${selectedSkills.length}.`
                ...
                <div className="flex flex-wrap gap-2 mt-2">
                  {selectedSkills.map((skill) => (
                    <div
                      key={skill}
                      className="flex items-center bg-blue-600 text-white px-3 py-1 rounded-full shadow-sm"
                    >
                      {skill}
                      <button
                        type="button"
